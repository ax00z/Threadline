...
[1/15/25, 3:01 PM] Fatima: around 6 probably
[1/15/25, 3:02 PM] Marcus: works for me
[1/15/25, 3:02 PM] Layla: same
"""
...
def test_bracket_24h():
    chat = "[12/3/24, 14:05] Reza: good afternoon\n"
    p = write_tmp(chat)
    try:
        msgs = list(parse_file(p))
        assert len(msgs) == 1
...
WA_DASH = """\
1/20/25, 9:10 AM - Nadia: morning
1/20/25, 9:11 AM - Tariq: hey
1/20/25, 9:15 AM - Nadia: did you see the news
"""
...
def test_system_messages_filtered():
    chat = (
        "[3/1/25, 8:00 AM] Kenji: hey\n"
        "[3/1/25, 8:01 AM] Kenji: Messages and calls are end-to-end encrypted. No one outside of this chat, not even WhatsApp, can read or listen to them.\n"
        "[3/1/25, 8:02 AM] Mei: hi\n"
    )
    p = write_tmp(chat)
    try:
        msgs = list(parse_file(p))
        assert len(msgs) == 2
        assert all(m.sender in ("Kenji", "Mei") for m in msgs)
    finally:
        os.unlink(p)
...
TELEGRAM_DATA = {
    ...
    "type": "personal_chat",
    "messages": [
        {"id": 1, "type": "message", "date": "2025-02-10T10:00:00", "from": "Darius", "text": "hello"},
        {"id": 2, "type": "message", "date": "2025-02-10T10:01:00", "from": "Yuki", "text": "hey"},
        {"id": 3, "type": "service", "date": "2025-02-10T10:02:00", "actor": "Darius", "text": ""},
        {"id": 4, "type": "message", "date": "2025-02-10T10:03:00", "from": "Darius", "text": [{"type": "bold", "text": "check this out"}]},
    ]
}
...
def test_telegram_list_format():
    data = [
        {"id": 1, "type": "message", "date": "2025-01-01T12:00:00", "from": "Ana", "text": "hi"},
    ]
...
CSV_DATA = "timestamp,sender,body\n2025-03-01T09:00:00,Priya,good morning\n2025-03-01T09:01:00,Leon,morning\n"
